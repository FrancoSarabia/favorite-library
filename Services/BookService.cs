    ...
    public class BookService : IBookService
    {
        ...
        public async Task<List<BookSearchResponseDto>> GetAllBooksAsync (string? title, string? author)
        {
            try
            {
                ...
                return openLibraryResponse?.Docs.Select(d => new BookSearchResponseDto
                {
                    ExternalId = d.ExternalIdBook ?? string.Empty,
                    Title = d.Title ?? "Unknown",
                    FirstPublishYear = d.FirstPublishYear,
                    Authors = d.Authors ?? new List<string>(),
                    CoverUrl = d.CoverId != null
                        ? $"https://covers.openlibrary.org/b/id/{d.CoverId}-L.jpg"
                        : null
